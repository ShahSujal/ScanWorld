      ...
      <div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        <div className="flex justify-start item-start space-y-2 flex-col ">
          <h1 className="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9  text-gray-800">
            Order #{Ordered[0].orderId}
          </h1>
          <p className="text-base font-medium leading-6 text-gray-600">
            {format(Ordered[0].createdAt)}
          </p>
        </div>
        <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch  w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
          <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
            <div className="flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
              ...
              {Ordered[0].product.map((element) => {
                console.log(element);
                console.log(element.itemcode.image);
                return (
                  <div className="div" key={element.itemcode}>
                    <div className="mt-6 md:mt-0 flex justify-start flex-col md:flex-row  items-start md:items-center space-y-4  md:space-x-6 xl:space-x-8 w-full ">
                      <div className="w-full md:w-40">
                        <img
                          className="w-full hidden md:block"
                          src={element.itemcode.image}
                          alt="dress"
                        />
                        <img
                ...
                <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row  items-center md:items-start ">
                  <div className="flex justify-center md:justify-start  items-center md:items-start flex-col space-y-4 xl:mt-8">
                    ...
                    <p className="w-48 lg:w-full xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">
                      {Ordered[0].address}, {Ordered[0].City}{" "}
                      {Ordered[0].Pincode}{" "}
                    </p>
                  </div>
                </div>
...
export async function getServerSideProps(context) {
  ...
  let Ordered = await Orders.find();

  // console.log("these is a jacket array"+ Object.keys(jacket));
  return {
    props: { Ordered: JSON.parse(JSON.stringify(Ordered)) }, // will be passed to the page component as props
  };
}
